# CodeMate 客户端工程 (codemate-frontend)
...
`codemate-frontend` 是 **CodeMate 智能编程协同平台** 的官方客户端工程，采用 **Vue 3（Composition API） + TypeScript + Vite** 构建，结合 **Vant UI** 与 **Tailwind CSS**，致力于为编程学习者与团队开发者打造流畅、高效且易用的移动端与响应式交互体验。
...
确保已安装 **Node.js 18+**，在 `codemate-frontend` 目录下执行：
```powershell
npm install
```
