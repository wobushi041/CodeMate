# CodeMate 服务端工程 (codemate-backend)
...
`codemate-backend` 是 **CodeMate 智能编程协同平台** 的服务端工程，基于 **Java 21** 与 **Spring Boot 3.5.3** 构建，深度整合 **Netty 4.1.118** 高性能异步网络框架，提供支撑平台业务流转的核心能力：
...
├── CodemateApplication.java               # Spring Boot 启动主类
...
# 2. 运行生产 JAR
java -jar -Dspring.profiles.active=prod .\target\codemate-backend.jar
```
生产环境部署推荐通过环境变量传入数据库及中间件密码：
```powershell
java -Dspring.datasource.password="your_pwd" -Dspring.data.redis.password="your_pwd" -jar .\target\codemate-backend.jar
```